...
class relative_depth_margin_log_negative_cos(nn.Module):
	"""docstring for relative_depth_negative_cos"""
	def __init__(self, w_normal, margin,camera):
		super(relative_depth_margin_log_negative_cos, self).__init__()
	...
	def forward(self, input, target):
		...
		if n_normal > 0:
			...
			self.loss_normal = self.w_normal*self.normal_crit(normal, target[1])
			output += self.loss_normal
...
if __name__ == '__main__':
	...
	crit = relative_depth_margin_log_negative_cos(1,3,camera).cuda()
