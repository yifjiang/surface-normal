...
def visualize_mask(mask, filename):
	mask_c = mask.clone()
	mask_c = mask_c - torch.min(mask_c)
	mask_c = mask_c / torch.max(mask_c)
	t_back = transforms.ToPILImage()
	mask_c = t_back(mask_c)

	# new_image = Image.new('RGB', (mask_c.size[0], mask_c.size[1]))
	# new_image.paste(mask_c, (0, 0))

	# new_image.save(filename)
	
	mask_c.save(filename)
